...
// const client = new Client("postgress://postgres:messi3214:localhost:5432/phenomena");
// const client = new Client({ connectionString: process.env.DATABASE_URL });

const CONNECTION_STRING = process.env.DATABASE_URL || "postgresql://postgres:messi3214@127.0.0.1:5432/phenomena"
...
async function getOpenReports() {
  try {
    ...
    const reportWithcomments = reports.map((report) => {
      const reportComments = comments.filter((comment)=> comment.reportId === report.id);
      return {
        ...report,
        comments: reportComments || [],
        isExpired: Date.parse(report.expirationDate) < Date.now(),
      };
    })
    const processedReports = reportWithcomments.map((report)=>{
      const {password, ...rest} = report;
      return rest;
    })
